...
note in the repo (identity + the marketing copy). **Values below are filled in for Sudoku.**

```
App name:        Sudoku
App ID (ASC):                             # numeric; resolved from bundle id if blank
Bundle ID:       com.tertiaryinfotech.sudokuapp
iCloud container: (none — no CloudKit; scores stored locally in UserDefaults)
Team ID:         GU9WTSTX9M               # Tertiary Infotech (paid) — same team as RunTrack GPS
...
> - **iPhone-only** (`TARGETED_DEVICE_FAMILY = 1`) — set in `project.yml`. The iPhone-screenshot
>   sets you need are `APP_IPHONE_67` (1290×2796) and/or `APP_IPHONE_65` (1242×2688).
...
```
Subtitle:    Classic number puzzles
Keywords:    sudoku,puzzle,number,logic,brain,board game,grid,classic,offline,hints
Promo text:  Unlimited Sudoku puzzles with four difficulty levels, smart hints, mistake
             tracking, pencil notes, and local high scores — all offline.
Description: Sudoku is a clean, offline number-puzzle game for grown-ups. Every puzzle is
             generated on your device with a guaranteed unique solution, so you never run
             out and never see the same grid twice. Choose from four difficulty levels —
             Easy, Medium, Hard, and Expert — and solve at your own pace.
             ...
             Track your best times and scores per difficulty, resume any game you leave,
             and train your brain one grid at a time.
```
